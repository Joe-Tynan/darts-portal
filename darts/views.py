...
class LeagueTableView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["monthly_leaders"] = CustomUser.objects.annotate(num_wins=Count('wins'), last_win=Max('wins__date')).filter(wins__date__month=datetime.date.today().month).order_by('-num_wins', 'last_win').distinct()[:3]
